...
It also carries the pipeline that feeds those readers: importing a
Strava export zip into a version-controlled repository, and converting
the archived activities to Parquet. Reports reading from the Parquet
...
- `derive_columns.R` – the `addcols_*` transforms:
  ...
  [`addcols_latlng_offset()`](https://torfason.github.io/starch/reference/derive_columns.md)
  (coordinates relative to the first fix, since the absolute values
  dwarf the within-activity variation). Pure tibble → tibble; documented
  as one family via `@describeIn`. Also holds `activity_col_order` and
  [`relocate_activity_cols()`](https://torfason.github.io/starch/reference/relocate_activity_cols.md).
- `strava_repo.R` – export import.
  [`latest_strava_zip()`](https://torfason.github.io/starch/reference/latest_strava_zip.md)
  picks the most recent archive from a directory;
...
with a clean working tree. That guard is what makes unattended
...
- **Long-running functions report progress.** Anything that moves
  ...
  guarded with `if (!quiet)` rather than suppressed wholesale.
...
[`relocate_activity_cols()`](https://torfason.github.io/starch/reference/relocate_activity_cols.md)
is the single authority on ordering; the `.after` arguments still
present in some `addcols_*` functions are vestigial and should not be
...
- Activity tests live in `test-read_streams.R` and
  ...
  `test-workouts.R`.
- Golden tests use `summarize_stream()` (in `helper-fixtures.R`) to
  reduce a stream to one row per column – name, NA count, mean, and a
  hash for character columns – and compare with
  `expect_snapshot_value(style = "json2")`, with snapshots under
  `_snaps/`. Comparing summaries rather than hashing whole columns is
  deliberate: `expect_equal()` semantics keep a numeric tolerance,
  whereas a digest of a double vector is a bit-exactness test that fails
...
- **Robust `speed` / `pace`.** The order reserves `speed` ahead of
  ...
  `dev_dist`) and falling back to computed – so that it is present
  whenever it can be inferred at all.
  [`addcols_speed()`](https://torfason.github.io/starch/reference/derive_columns.md)
...
- **Tests for the pipeline.** Neither
  ...
  contains personal data. The invariant worth asserting is idempotence:
  run twice, and the second run extracts nothing, converts nothing, and
  makes no commit.
- **One tree walk too many.**
  [`strava_zip_to_repo()`](https://torfason.github.io/starch/reference/strava_zip_to_repo.md)
  calls `git_status()` before staging purely to count changes for the
  commit message. Running `git_add(".")` first and then
  `git_status(staged = TRUE)` would cut a full walk, which is minutes on
  a large import.
...
- **Empty streams.** A recordless FIT file reads as `0 x 0`, which has
  ...
  `nanoparquet` actually does with a zero-column frame.
- **Offsets in Parquet.** `lat_offset` / `lng_offset` are an inspection
  aid but are currently persisted, keeping the Parquet shape identical
...
- **Deleted activities.** Activities deleted in Strava vanish from later
  exports but persist in the repository. Archive semantics argue for
  keeping them, but it is a silent divergence from Strava’s state.
- **`commit = FALSE`** leaves a dirty working tree, which blocks the
  next import until it is resolved. Intended, but it makes the flag
...
- gzip byte-stability holds **within** a machine, not across them: the
  OS byte is fixed at build time and the deflate stream can differ
